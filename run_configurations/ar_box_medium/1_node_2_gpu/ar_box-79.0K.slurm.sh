...
#SBATCH -A stf006
#SBATCH -J pc-1-node-2-gpu-0_ar_medium_box-solid_liquid-double_precision-79.0K-79.0K
#SBATCH -o %x-%j.out
#SBATCH -e %x-%j.err
#SBATCH -t 0:30:00
#SBATCH -N 1
...
declare -r exp_nm="1-node-2-gpu"
declare -r run_nm="0"
declare -r prec="double_precision"
declare -r box_size="ar_medium_box"
declare -r parent_work_dir="ar_box_medium_for_wael"
...
declare -r number_tasks_per_node=2
...
srun --nodes "${number_nodes}" \
  ...
  --cpu-bind=threads \
  -m block:cyclic \
  "${BIN}" -l ${lammps_log_file} -kokkos on g 2 -sf kk -in "${command_file}"
